...
window.addEventListener("online", () => {
  ...
  showTemporaryMessage("You are back online!", "success");
});
...
// Update the api function for offline support
async function api(path, options = {}) {
  const headers = options.headers || {};
  const token = getToken();
  ...
  // Check if we're offline and it's a modifying request
  ...
  try {
    ...
    if (!navigator.onLine && options.method === "GET") {
      const cache = await caches.open("hr-onboarding-api-v1");
      const cachedResponse = await cache.match(path);
      if (cachedResponse) {
        const data = await cachedResponse.json();
        showTemporaryMessage("Showing cached data (offline mode)", "info");
        return data;
      }
    }
...
function showInstallPrompt() {
  ...
  promptDiv.innerHTML = `
    <span>📱 Install HR Onboarding app?</span>
  ...
  promptDiv.querySelector(".close").addEventListener("click", () => {
    promptDiv.remove();
  });
}
