...
async function api(path, options = {}) {
  ...
  // If body is NOT FormData, default JSON
...
// -------- existing auth flows (keep yours) --------
...
async function loadChecklistPage() {
  ...
  try {
    ...
    for (const it of data.items) {
      ...
      row.innerHTML = `
        <div>
          <div style="font-weight:800">${it.Title} <span class="pill" style="margin-left:8px">${it.Stage}</span></div>
          <div style="color:#475569;font-size:13px">${it.Description || ""}</div>
        </div>
        <div style="display:flex;gap:8px;align-items:center">
          <span class="badge">${it.Status}</span>
          <button class="btn ${it.Status === "DONE" ? "" : "btn-primary"}" data-id="${it.ItemId}">
  ...
  try {
    ...
    for (const d of data.documents) {
      ...
      tr.innerHTML = `
        ...
        <td><a href="${d.FileUrl}" target="_blank" style="text-decoration:underline">View</a></td>
        <td>
          <input id="c-${d.DocId}" placeholder="Comment (optional)" style="padding:8px;border-radius:10px;border:1px solid var(--border);width:220px;">
        </td>
  ...
  try {
    ...
    for (const t of data.trainings) {
      ...
      tr.innerHTML = `
        ...
        <td>${t.Attendance}</td>
        <td>
          <button class="btn ${t.Attendance === "ATTENDED" ? "" : "btn-primary"}" data-id="${t.TrainingId}">
